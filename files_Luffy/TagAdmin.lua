...
local function keko_Luffy(data)
    ...
    local var = false
    local mod = database:sismember('Luffy:'..bot_id..'mods:'..chat_id, user_id)  
    local admin = database:sismember('Luffy:'..bot_id..'admins:', user_id)  
    local owner = database:sismember('Luffy:'..bot_id..'owners:'..chat_id, user_id)
    local creator = database:sismember('Luffy:'..bot_id..'creator:'..chat_id, user_id)  
    local vip = database:sismember('Luffy:'..bot_id..'vip'..chat_id, user_id)
    ...
    -----------------------------
    if text and text == "تفعيل تاك الادمنيه" and database:sismember('Luffy:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) then 
    database:set("Luffy:tag:admin:"..bot_id..msg.chat_id_,"Luffy")
    send(msg.chat_id_, msg.id_, 1, "⚠️┇تم تفعيل تاك الادمنيه", 1, 'html')
    end
    if text and text == "تعطيل تاك الادمنيه" and database:sismember('Luffy:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) then 
    database:del("Luffy:tag:admin:"..bot_id..msg.chat_id_)
    send(msg.chat_id_, msg.id_, 1, "⚠️┇تم تعطيل تاك الادمنيه", 1, 'html')
    end
    if (text and ( text == 'صيح الادمنيه' or text == "تاك للادمنيه" or text == "وين الادمنيه") and (is_vip(msg) or database:get("Luffy:tag:admin:"..bot_id..msg.chat_id_))) then 
    function cb(t1,t2)
    ...
    i = i + 1
    local user_info = database:hgetall('Luffy:'..bot_id..'user:'..v.user_id_)
    ...
    send(msg.chat_id_, msg.id_, 1, new_text, 1, 'Luffy')
    end
